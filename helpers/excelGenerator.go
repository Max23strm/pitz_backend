...
func CreateExpensesExcel(expenses models.ExpensesFileRows, monthlyPayments models.MonthlyFileRows) (*excelize.File, error) {
	...
	f.SetCellValue("Sheet1", "C1", "Apellido")
	f.SetCellValue("Sheet1", "D1", "Razón de gasto")
	f.SetCellValue("Sheet1", "E1", "Monto")

	f.SetCellValue("Sheet1", "H1", "Mes")
	f.SetCellValue("Sheet1", "I1", "Total de Mes")

	f.SetCellStyle("Sheet1", "A1", "E1", headerStyle)
	f.SetCellStyle("Sheet1", "H1", "I1", headerStyle)

	totalAmount := 0.00

	totalRow := strconv.Itoa(len(expenses) + 3)
	for i, payment := range expenses {
		currentRow := strconv.Itoa(i + 2)

		f.SetCellValue("Sheet1", "A"+currentRow, payment.Date)
		f.SetCellValue("Sheet1", "B"+currentRow, payment.Assigned_first_name)
		f.SetCellValue("Sheet1", "C"+currentRow, payment.Assigned_last_name)
		f.SetCellValue("Sheet1", "D"+currentRow, payment.Reason)
		f.SetCellValue("Sheet1", "E"+currentRow, payment.Amount)

		f.SetCellStyle("Sheet1", "E"+currentRow, "E"+currentRow, numberStyle)
		f.SetCellStyle("Sheet1", "A"+currentRow, "A"+currentRow, dateStyle)
		totalAmount = totalAmount + payment.Amount
	...
	for i, month := range monthlyPayments {
		currentRow := strconv.Itoa(i + 2)
		f.SetCellValue("Sheet1", "H"+currentRow, month.Month)
		f.SetCellValue("Sheet1", "I"+currentRow, month.Amount)

		f.SetCellStyle("Sheet1", "I"+currentRow, "I"+currentRow, numberStyle)
		f.SetCellStyle("Sheet1", "H"+currentRow, "H"+currentRow, monthStyle)

	}

	f.SetCellValue("Sheet1", "A"+totalRow, "Total")
	f.SetCellValue("Sheet1", "E"+totalRow, totalAmount)
	f.SetCellStyle("Sheet1", "A"+totalRow, "E"+totalRow, headerStyle)
